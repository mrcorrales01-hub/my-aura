-- CRITICAL SECURITY FIX: Encrypt User Profile Data & Emergency Contacts (Fixed Syntax)
...
-- Add encrypted fields for safety networks
...
AS $$
  ...
  WHERE sn.user_id = p_user_id
    ...
    AND auth.uid() = p_user_id; -- Only user's own data
$$;
