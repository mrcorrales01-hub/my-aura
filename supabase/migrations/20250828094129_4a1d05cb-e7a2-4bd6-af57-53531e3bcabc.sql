-- Security Fix Migration: Comprehensive Database Security Hardening (Fixed)
...
-- 10. Create session security audit function (for INSERT/UPDATE/DELETE only)
...
-- Add security audit triggers to sensitive tables (INSERT/UPDATE/DELETE only)
DROP TRIGGER IF EXISTS audit_profiles_changes ON public.profiles;
CREATE TRIGGER audit_profiles_changes
  AFTER INSERT OR UPDATE OR DELETE ON public.profiles
  FOR EACH ROW EXECUTE FUNCTION public.audit_sensitive_data_changes();

DROP TRIGGER IF EXISTS audit_therapists_changes ON public.therapists;  
CREATE TRIGGER audit_therapists_changes
  AFTER INSERT OR UPDATE OR DELETE ON public.therapists
  FOR EACH ROW EXECUTE FUNCTION public.audit_sensitive_data_changes();

DROP TRIGGER IF EXISTS audit_safety_networks_changes ON public.safety_networks;
CREATE TRIGGER audit_safety_networks_changes
