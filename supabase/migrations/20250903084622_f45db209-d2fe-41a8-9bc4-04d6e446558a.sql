...
-- Create function for secure user profile access
CREATE OR REPLACE FUNCTION public.get_secure_user_profile(p_user_id uuid DEFAULT NULL)
RETURNS TABLE(
  id uuid,
  full_name text,
...
AS $$
  ...
  WHERE p.id = COALESCE(p_user_id, auth.uid())
    AND (p.id = auth.uid() OR is_system_admin());
$$;
...
-- Create function to audit data access patterns
CREATE OR REPLACE FUNCTION public.audit_sensitive_access(table_name text, record_id uuid, access_type text)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  -- Calculate risk based on table sensitivity
  risk_score := CASE 
    WHEN table_name IN ('therapists', 'appointments', 'safety_networks') THEN 60
    WHEN table_name IN ('profiles', 'conversations', 'mood_entries') THEN 40
    WHEN table_name IN ('community_posts', 'community_comments') THEN 20
    ELSE 10
  END;
  
  -- Increase risk for bulk access patterns
  IF access_type = 'bulk_read' THEN risk_score := risk_score + 30; END IF;
  IF access_type = 'export' THEN risk_score := risk_score + 40; END IF;
...
-- Create trigger for automatic sensitive data access auditing
...
AS $$
BEGIN
  -- Only audit on sensitive tables
  IF TG_TABLE_NAME IN ('therapists', 'appointments', 'safety_networks', 'profiles', 'conversations') THEN
...
DROP TRIGGER IF EXISTS audit_therapists_access ON therapists;
CREATE TRIGGER audit_therapists_access
  AFTER SELECT ON therapists
  FOR EACH STATEMENT
  EXECUTE FUNCTION auto_audit_trigger();
