-- Ultra Patch 2.0 Database Tables (Fixed)
...
CREATE TABLE public.group_vr_sessions (
  ...
  world_id UUID REFERENCES public.vr_therapy_worlds(id),
  facilitator_type TEXT DEFAULT 'ai',
  facilitator_id UUID,
...
-- Enhanced Daily Micro-Challenges
...
-- Family Supporter Enhanced Sessions
...
ON public.biofeedback_sessions FOR ALL 
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Family members can view supporter sessions" 
ON public.family_supporter_sessions FOR SELECT 
USING (
  (auth.uid() = supporter_user_id) OR 
  (auth.uid() = supported_user_id) OR 
  (family_account_id IN (
    SELECT family_account_id FROM profiles WHERE id = auth.uid()
  ))
);

CREATE POLICY "Family supporters can create sessions" 
ON public.family_supporter_sessions FOR INSERT 
WITH CHECK (
