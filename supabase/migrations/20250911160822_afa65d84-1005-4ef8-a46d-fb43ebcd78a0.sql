...
-- Add similar restrictions to other content tables
...
-- Fix 5: Add data encryption helpers for profiles
CREATE OR REPLACE FUNCTION public.encrypt_profile_field(field_value text)
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
AS $function$
BEGIN
  -- Simple masking for now - in production, use proper encryption
  IF field_value IS NULL OR length(field_value) < 3 THEN
    RETURN field_value;
  END IF;
  
  -- Mask middle characters, keep first and last
  RETURN left(field_value, 1) || repeat('*', greatest(length(field_value) - 2, 1)) || right(field_value, 1);
END;
$function$;

-- Fix 6: Enhanced audit logging for sensitive data access
...
-- Add triggers for sensitive data access monitoring
DROP TRIGGER IF EXISTS audit_therapist_access ON public.therapists;
CREATE TRIGGER audit_therapist_access
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.therapists
  FOR EACH ROW EXECUTE FUNCTION audit_sensitive_data_access_v2();
