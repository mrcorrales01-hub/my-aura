...
-- Create separate triggers for INSERT and DELETE on likes
CREATE TRIGGER trigger_insert_post_like_count
  AFTER INSERT ON public.community_likes
  FOR EACH ROW
  WHEN (NEW.post_id IS NOT NULL)
  EXECUTE FUNCTION update_post_like_count();

CREATE TRIGGER trigger_delete_post_like_count
  AFTER DELETE ON public.community_likes
  FOR EACH ROW
  WHEN (OLD.post_id IS NOT NULL)
  EXECUTE FUNCTION update_post_like_count();

CREATE TRIGGER trigger_insert_comment_like_count
  AFTER INSERT ON public.community_likes
  FOR EACH ROW
  WHEN (NEW.comment_id IS NOT NULL)
  EXECUTE FUNCTION update_comment_like_count();

CREATE TRIGGER trigger_delete_comment_like_count
  AFTER DELETE ON public.community_likes
  FOR EACH ROW
  WHEN (OLD.comment_id IS NOT NULL)
  EXECUTE FUNCTION update_comment_like_count();

CREATE TRIGGER trigger_insert_post_comment_count
  AFTER INSERT ON public.community_comments
  FOR EACH ROW
  EXECUTE FUNCTION update_post_comment_count();

CREATE TRIGGER trigger_delete_post_comment_count
